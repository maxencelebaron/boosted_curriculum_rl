...
def run_analysis(datasets, outdir, title="Car-on-Hill datasets"):
    """
    datasets : dict  {m (float) -> list of transitions}
    outdir   : str   folder where .txt and .png are saved
    """
    ...
    for m, dataset in datasets.items():
        ...
        lines += [
            ...
            f"  Rewards > 0 (+1)   : {pos_rew}  ({100*pos_rew/n:.4f}%)",
            f"  Rewards < 0 (-1)   : {neg_rew}  ({100*neg_rew/n:.4f}%)",
            f"  Rewards = 0        : {zero_rew} ({100*zero_rew/n:.4f}%)",
    ...
    with open(f"{outdir}/dataset_analysis.txt", "w") as f:
        f.write("\n".join(lines) + "\n")
    print("\n".join(lines))
    ...
    plt.savefig(f"{outdir}/dataset_analysis.png", dpi=150, bbox_inches='tight')
    plt.close()
