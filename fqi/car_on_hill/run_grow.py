...
GROWTH_MODULES = {
    ...
    "gromo_all_layer": "fqi.6_network_fqi_tiny_all_layer",
}
...
class Args:
    ...
    growth_mode: str = "gromo_one_layer"
    """growth strategy: random | svd | gromo_one_layer | gromo_all_layer"""
    ...
    n_growth_steps: int = 5
    """number of growth events evenly spaced over iters_per_env"""
    pre_growth_steps: int = 100
    """gradient steps to saturate current arch before growing (0 = skip)"""
    ...
    neurons_to_add: int = 8
    """neurons added per step (random / svd only)"""
    maximum_added_neurons: int | None = None
    """max neurons gromo may add (None = uncapped)"""
...
def _compute_grow_batch(
    ...
    """Sample a random sub-batch from dataset and compute TD targets."""
    ...
    q_next = regressor.predict(s_next)          # (batch, n_actions)
...
def _grow_step(
    ...
    """
    Optionally saturate the current arch, then grow according to growth_mode.
    ...
    if args.pre_growth_steps > 0:
        ...
        _, final_loss = pre_growth_optimize(
    ...
    if growth_mode == "random":
        new_h = q_network.encoder[0].out_features + args.neurons_to_add
        new_net = module.grow_network(q_network, new_h)
        regressor._model = new_net
        regressor._optimizer = None

    elif growth_mode == "svd":
        new_net, _ = module.grow_network_svd(
            q_network, states, actions, td_targets,
            d_a=args.neurons_to_add,
    ...
    elif growth_mode == "gromo_all_layer":
        module.grow_layer_gromo(
            q_network, states, actions, td_targets,
...
def experiment(exp_id: int, args: Args) -> tuple:
    ...
    mdps = [CarOnHill() for _ in range(ms)]
    ...
    growth_freq = max(1, args.iters_per_env // args.n_growth_steps)
    ...
    for i, mdp in enumerate(mdps):
        ...
        for it in trange(args.iters_per_env, dynamic_ncols=True, leave=False):
            ...
            # Growth step every growth_freq iterations
            if (it + 1) % growth_freq == 0:
...
if __name__ == '__main__':
    ...
    folder_name = (
        f'./logs/grow_{args.growth_mode}_{boost}_{cur}'
        f'_h{args.initial_hidden}_g{args.n_growth_steps}'
