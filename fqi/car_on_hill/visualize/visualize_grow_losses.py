...
def plot_curves(
    ...
    if use_curriculum:
        ...
        for task in range(n_tasks):
            ...
            colors = task_cmaps[task % len(task_cmaps)](
                np.linspace(0.35, 0.95, iters_per_task)
            )
        ...
        for idx in range(n_iters):
            mean_curve = np.mean(data[:, idx, :], axis=0)
            line, = ax.plot(
                epochs, mean_curve, color=colors[idx], alpha=0.8, linewidth=0.7
            )
...
def visualize_loss_convergence(folder, algo_label, n_tasks, path=None, fontsize=9, ticksize=8):
    ...
    plot_curves(axes[1], q_errors, n_tasks, use_curriculum,
                ylabel=r"$\| Q_t^{k_j} - Q_t^* \|_{1, \mu}$",
                show_task_title=False, fontsize=fontsize, ticksize=ticksize)
...
if __name__ == "__main__":
    ...
    for subfolder, n_tasks in [
        ("logs/logs_curriculum",    3),
        ("logs/logs_no_curriculum", 1),
    ]:
        all_folders = sorted(glob.glob(os.path.join(subfolder, "*")))
        all_folders = [f for f in all_folders if os.path.isdir(f)]

        for folder in all_folders:
            losses_path = os.path.join(folder, "losses.npy")
            ...
            visualize_loss_convergence(
                ...
                path=f"figures/losses_{name}.pdf",
