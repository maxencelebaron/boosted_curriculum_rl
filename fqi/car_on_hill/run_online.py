...
def experiment(
    ...
    for i, mdp in enumerate(mdps):
        ...
        # Dataset collection before resetting the agent, so the previous policy is intact
        try:
            with open('%s/dataset_%1.3f.pkl' % (data_dir, mdp._m), 'rb') as f:
                dataset = pickle.load(f)
        except FileNotFoundError:
            if i == 0 or not prev_trained:
                agent.policy.set_epsilon(epsilon)
                logger.info('Generating dataset for task %d with random policy...' % i)
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--epsilon", type=float, default=0.1,
                        help="Epsilon for dataset collection with the trained policy")
    parser.add_argument("--n-episodes", type=int, default=1000,
                        help="Number of episodes to collect per task dataset")
    args = parser.parse_args()

    collect_eps_param = Parameter(value=args.epsilon)
